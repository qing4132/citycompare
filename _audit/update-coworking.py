...
has_cw = sum(1 for c in compiled['cities'].values() 
...
scores = sorted([c['coworking']['compositeScore'] for c in compiled['cities'].values() 
                if isinstance(c.get('coworking'), dict) and c['coworking'].get('compositeScore') is not None])
print(f'Range: {scores[0]:.2f} - {scores[-1]:.2f}, Median: {scores[len(scores)//2]:.2f}')

ranked = sorted(
    [(c['en'], c['coworking']['compositeScore']) for c in compiled['cities'].values() 
     if isinstance(c.get('coworking'), dict) and c['coworking'].get('compositeScore') is not None],
