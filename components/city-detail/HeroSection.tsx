...
export default function HeroSection({ city, cityName, countryName, flag, slug, locale, darkMode, t }: Props) {
    ...
    return (
        <div className={`pb-4 border-b ${divider}`}>
            {city.safetyWarning && (
                <div className={`rounded-lg px-4 py-2 mb-4 text-sm flex items-start gap-2 ${city.safetyWarning === "active_conflict"
            ...
            <div className="flex items-center gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                    <h1 className={`text-xl font-black ${headCls}`}>{cityName}</h1>
                    <p className={`text-[10px] ${subCls}`}>@{slug} · {countryName} · {CONTINENT_TRANSLATIONS[city.continent]?.[locale as Locale] || city.continent}</p>
                </div>
                <Link href={`/${locale}/compare/${slug}`}
                    className={`text-[10px] px-3 py-1 rounded-md font-semibold shrink-0 ${darkMode ? "bg-slate-800 text-slate-200" : "bg-slate-900 text-white"}`}>
                    + {t("navCompare")}
                </Link>
            </div>
            {CITY_INTROS[city.id] && (
